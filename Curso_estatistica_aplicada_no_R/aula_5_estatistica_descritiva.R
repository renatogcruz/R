...
#dados <- read.csv('Dados_a4.csv', sep = ';', dec = ',')

#dados <- read.csv2('Dados_a4.csv') # já tem como padrão "sep = ';', dec = ','"
...
range(dados$Salario) # qual menor e maoir salário deste banco de dados?
...
table(cut(dados$Salario, seq(0, 6, l = 7))) # output == "( <- não incluido, [ <- incluido"
...
describeBy(dados$Salario, group = list(dados$Genero,dados$Grau_de_Instruçao))
